...
.登录
image:login.png[] 默认用户名/密码 `root@weghst.com/root`.

.添加用户
image:user_add_1.png[]

Setaria 提供2种用户类型, 管理员拥有所有 Console 操作权限, 普通用户只可管理属于自己的应用配置.

.用户列表
image:user_list_2.png[]
...
.查看应用客户端连接信息
image:app_list_4.png[]

点击应用 ID 可看应用客户端连接信息.

image:app_client_infos_1.png[]

.应用配置列表
image:app_list_3.png[]

点击应用名称可进入应用配置列表

image:config_list_1.png[]

NOTE: 配置列表右上角可导入/导出当前配置信息, 导入时会根据配置键自动匹配现有配置, 如果键已经存在则会覆盖反之则会增加配置.

.添加配置
image:config_add_1.png[]

.修改配置信息
image:config_update_1.png[]
